...
<div class="min-h-screen bg-gradient-to-br from-teh-jawa-cream via-white to-teh-jawa-gold-accent/20">
    <div class="container mx-auto px-4 py-6 md:py-8 max-w-4xl">
        ...
        <h1 class="text-4xl font-bold text-teh-jawa-black mb-2">Buat Pesanan Baru</h1>
        <p class="text-teh-jawa-gray mb-8">Pilih menu dan buat pesanan Anda</p>
        ...
        <form method="POST" action="{{ route('orders.store') }}" class="space-y-6" id="orderForm">
                    ...
                    <div class="orderItem bg-gray-50 p-4 rounded-lg border-2 border-gray-200">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="items[0][menu_item_id]" class="input-teh w-full menuSelect" required>
                            ...
                            <div>
                                ...
                                <input type="number" name="items[0][quantity]" class="input-teh w-full" min="1" value="1" required>
                            </div>
                            <div class="md:col-span-2">
                                <label class="label-teh">Catatan Spesial (Topping, dll)</label>
                                <input type="text" name="items[0][notes]" class="input-teh w-full" placeholder="Contoh: Topping Ayam, No Gula...">
                            </div>
            ...
            <div class="flex gap-4 justify-end">
                <a href="{{ route('orders.index') }}" class="btn-teh-secondary">Batal</a>
                <button type="submit" class="btn-teh-primary">Pesan Sekarang</button>
            </div>
        </form>
    </div>
</div>
...
document.querySelectorAll('.orderItem').forEach((item, idx) => {
    if(idx > 0) item.querySelector('.removeItem').style.display = 'block';
});
attachRemoveListeners();
</script>
@endsection
